...
#config
app="indexer"
appMainClass=com.zero.tech.metrics.MetricsApplication

export METRICS_HOME="$cur"
APP_HOME="$METRICS_HOME"
javaHome=$JAVA_HOME
if [[ -z "$javaHome" ]]; then
...
if [[ $# == 1 ]];  then
    ...
    if [[ "$environment" = "test" ]];then
        ...
        DEBUG="-Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=11185"
